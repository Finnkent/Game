...
with TelegramClient(sesi_file, api_id, api_hash) as client:
    client.loop.run_until_complete(client.send_message(-1001943370596 , lanjut))
    ...
    async def handle_chat(event):
        ...
        if "Anda adalah 🤵🏻 Boss Lana" in pesan or "Anda adalah 🤵🏼 Mafia!" in pesan:
            time.sleep(5)
            await client.send_message(-1001943370596,quit)
        ...
        if "Anda adalah 👨🏼 Warga" in pesan:
            time.sleep(7)
            await client.send_message(-1001943370596 ,lanjut)
        ...
        elif "Permainan telah berakhir" in pesan:
            print(time.asctime(), pesan)
            return

        
    @client.on(events.NewMessage(chats=-1001943370596))
